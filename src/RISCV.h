...
std::string reg_names[16] = {"t0", "t1", "t2", "t3", "t4", "t5", "t6", "a0",
                             "a1", "a2", "a3", "a4", "a5", "a6", "a7", "x0"};
...
void visit(const koopa_raw_function_t& func) {
    ...
    if (stack_frame_size > 0 && stack_frame_size * 4 <= 2048)
        ...
        std::cout << "\tli    " << reg_names[tmp_var.reg_name] << ", -" << stack_frame_size * 4
                  << std::endl;
        std::cout << "\taddi  sp, sp, " << reg_names[tmp_var.reg_name] << std::endl;
    }
    visit(func->bbs);
    stack_size -= stack_frame_size * 4;
...
void visit(const koopa_raw_basic_block_t& basic_block) {
    std::cout << basic_block->name + 1 << ":" << std::endl;
...
Reg visit(const koopa_raw_value_t& value) {
    ...
    if (value_map.count(value)) {
        if (value_map[value].reg_name == -1) {
            ...
            std::cout << "\tlw    " << reg_names[reg_name] << ", " << value_map[value].reg_offset
                      << "(sp)" << std::endl;
        }
...
void visit(const koopa_raw_return_t& ret) {
    ...
    struct Reg result_var = visit(value);
...
Reg visit(const koopa_raw_binary_t& binary) {
    ...
    switch (binary.op) {
        case 0: // ne
            if (right_name == "x0") {
                std::cout << "\tsnez  " << result_name << ", " << left_name << std::endl;
                break;
            }
            if (left_name == "x0") {
                std::cout << "\tsnez  " << result_name << ", " << right_name << std::endl;
                break;
            }
            std::cout << "\txor   " << result_name << ", " << left_name << ", " << right_name
                      << std::endl;
            std::cout << "\tsnez  " << result_name << ", " << result_name << std::endl;
            break;
        case 1: // eq
            if (right_name == "x0") {
                std::cout << "\tseqz  " << result_name << ", " << left_name << std::endl;
                break;
            }
            if (left_name == "x0") {
                std::cout << "\tseqz  " << result_name << ", " << right_name << std::endl;
                break;
            }
            std::cout << "\txor   " << result_name << ", " << left_name << ", " << right_name
                      << std::endl;
            ...
            break;
        case 2: // gt
            std::cout << "\tsgt   " << result_name << ", " << left_name << ", " << right_name
                      << std::endl;
            break;
        case 3: // lt
            std::cout << "\tslt   " << result_name << ", " << left_name << ", " << right_name
                      << std::endl;
            break;
        case 4: // ge
            std::cout << "\tslt   " << result_name << ", " << left_name << ", " << right_name
        ...
        case 5: // le
            ...
            break;
        case 6: // add
            std::cout << "\tadd   " << result_name << ", " << left_name << ", " << right_name
                      << std::endl;
            break;
        case 7: // sub
            std::cout << "\tsub   " << result_name << ", " << left_name << ", " << right_name
                      << std::endl;
            break;
        case 8: // mul
            std::cout << "\tmul   " << result_name << ", " << left_name << ", " << right_name
                      << std::endl;
            break;
        case 9: // div
            std::cout << "\tdiv   " << result_name << ", " << left_name << ", " << right_name
                      << std::endl;
            break;
        case 10: // mod
            std::cout << "\trem   " << result_name << ", " << left_name << ", " << right_name
                      << std::endl;
            break;
        case 11: // and
            std::cout << "\tand   " << result_name << ", " << left_name << ", " << right_name
                      << std::endl;
            break;
        case 12: // or
            std::cout << "\tor    " << result_name << ", " << left_name << ", " << right_name
                      << std::endl;
            break;
        default:
            assert(false);
...
Reg visit(const koopa_raw_load_t& load) {
    ...
    if (reg_offset >= -2048 && reg_offset <= 2047)
        ...
        struct Reg tmp_var = {-1, -1};
        tmp_var.reg_name = find_reg(0);
        std::cout << "\tli    " << reg_names[tmp_var.reg_name] << ", " << reg_offset << std::endl;
        std::cout << "\tadd   " << reg_names[tmp_var.reg_name] << ", "
                  << reg_names[tmp_var.reg_name] << ", sp" << std::endl;
        std::cout << "\tlw    " << reg_names[reg_name] << ", (" << reg_names[tmp_var.reg_name]
                  << ")" << std::endl;
    }
    return result_var;
}
...
void visit(const koopa_raw_store_t& store) {
    ...
    if (reg_offset >= -2048 && reg_offset <= 2047)
        ...
        std::cout << "\tadd   " << reg_names[tmp_var.reg_name] << ", "
                  << reg_names[tmp_var.reg_name] << ", sp" << std::endl;
        std::cout << "\tsw    " << reg_names[reg_name] << ", (" << reg_names[tmp_var.reg_name]
                  << ")" << std::endl;
    }
}
    ...
    for (int i = 0; i < 15; i++) {
        if (reg_stats[i] == 1) {
            ...
            if (offset >= -2048 && offset <= 2047)
                std::cout << "\tsw    " << reg_names[i] << ", " << offset << "(sp)" << std::endl;
            else {
                std::cout << "\tli    s11, " << offset << std::endl;
                std::cout << "\tadd   s11, s11, sp" << std::endl;
                std::cout << "\tsw    " << reg_names[i] << ", (s11)" << std::endl;
            }
            registers[i] = present_value;
            reg_stats[i] = stat;
            return i;
