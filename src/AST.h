  ...
  public:
    ...
    std::string dumpIR() const override {
        if (stmt_type == StmtType::simple) {
            ...
            std::cout << "\tbr " << cond_result << ", " << then_label << ", " << end_label
                      << std::endl;
            std::cout << then_label << ":" << std::endl;
            std::string if_stmt_type = if_stmt->dumpIR();
            if (if_stmt_type != "ret" && if_stmt_type != "break" && if_stmt_type != "cont")
            ...
            std::string end_label = "\%end_" + std::to_string(if_else_num++);
            std::cout << "\tbr " << cond_result << ", " << then_label << ", " << else_label
                      << std::endl;
            std::cout << then_label << ":" << std::endl;
            std::string if_stmt_type = if_stmt->dumpIR();
            if (if_stmt_type != "ret" && if_stmt_type != "break" && if_stmt_type != "cont")
  ...
  public:
    ...
    std::string dumpIR() const override {
        if (type == SimpleStmtType::ret) {
            ...
            std::cout << "\tstore " << result_var << ", " << std::get<std::string>(value)
                      << std::endl;
        } else if (type == SimpleStmtType::exp) {
            if (block_exp != nullptr) {
                block_exp->dumpIR();
...
class VarDefAST : public BaseAST {
  public:
    ...
    std::string dumpIR() const override {
        ...
        std::string name = var_name + "_" + std::to_string(var_num[var_name]++);
  ...
  public:
    ...
    std::string dumpIR() const override {
        ...
        if (op == "")
            ...
            std::cout << "\tbr " << left_result << ", " << then_label << ", " << else_label
                      << std::endl;
            std::cout << then_label << ":" << std::endl;
...
class EqExpAST : public BaseAST {
  public:
    ...
    std::string dumpIR() const override {
        std::string result_var = "";
        if (op == "")
            result_var = rel_exp->dumpIR();
        else {
            std::string left_result = eq_exp->dumpIR();
            std::string right_result = rel_exp->dumpIR();
            result_var = "%" + std::to_string(symbol_num++);
            if (op == "==")
                std::cout << '\t' << result_var << " = eq " << left_result << ", " << right_result
                          << std::endl;
            else if (op == "!=")
                std::cout << '\t' << result_var << " = ne " << left_result << ", " << right_result
                          << std::endl;
            else
                assert(false);
        }
...
class RelExpAST : public BaseAST {
  public:
    ...
    std::string dumpIR() const override {
        std::string result_var = "";
        if (op == "")
            result_var = add_exp->dumpIR();
        else {
            std::string left_result = rel_exp->dumpIR();
            std::string right_result = add_exp->dumpIR();
            result_var = "%" + std::to_string(symbol_num++);
            if (op == "<")
                std::cout << '\t' << result_var << " = lt " << left_result << ", " << right_result
                          << std::endl;
            else if (op == ">")
                std::cout << '\t' << result_var << " = gt " << left_result << ", " << right_result
                          << std::endl;
            else if (op == "<=")
                std::cout << '\t' << result_var << " = le " << left_result << ", " << right_result
                          << std::endl;
            else if (op == ">=")
                std::cout << '\t' << result_var << " = ge " << left_result << ", " << right_result
                          << std::endl;
            else
                assert(false);
        }
...
class AddExpAST : public BaseAST {
  public:
    ...
    std::string dumpIR() const override {
        std::string result_var = "";
        if (op == "")
            result_var = mul_exp->dumpIR();
        else {
            std::string left_result = add_exp->dumpIR();
            std::string right_result = mul_exp->dumpIR();
            result_var = "%" + std::to_string(symbol_num++);
            if (op == "+")
                std::cout << '\t' << result_var << " = add " << left_result << ", " << right_result
                          << std::endl;
            else if (op == "-")
                std::cout << '\t' << result_var << " = sub " << left_result << ", " << right_result
                          << std::endl;
            else
                assert(false);
        }
...
class MulExpAST : public BaseAST {
  public:
    ...
    std::string dumpIR() const override {
        std::string result_var = "";
        if (op == "")
            result_var = unary_exp->dumpIR();
        else {
            std::string left_result = mul_exp->dumpIR();
            std::string right_result = unary_exp->dumpIR();
            result_var = "%" + std::to_string(symbol_num++);
            if (op == "*")
                std::cout << '\t' << result_var << " = mul " << left_result << ", " << right_result
                          << std::endl;
            else if (op == "/")
                std::cout << '\t' << result_var << " = div " << left_result << ", " << right_result
                          << std::endl;
            else if (op == "%")
                std::cout << '\t' << result_var << " = mod " << left_result << ", " << right_result
                          << std::endl;
            else
                assert(false);
        }
    ...
    std::string dumpIR() const override {
        ...
        if (type == PrimaryExpType::exp)
            ...
            if (value.index() == 0)
                ...
                result_var = "%" + std::to_string(symbol_num++);
                std::cout << '\t' << result_var << " = load " << std::get<std::string>(value)
                          << std::endl;
            }
        } else
            assert(false);
